...
To improve data quality, the combined **Talking\_Yawning** class was removed because of its limited number of samples, resulting in three balanced classes.
...
The optimized MobileNetV3 model achieved approximately **94% classification accuracy** on the validation dataset, demonstrating its suitability for real-time driver activity recognition.

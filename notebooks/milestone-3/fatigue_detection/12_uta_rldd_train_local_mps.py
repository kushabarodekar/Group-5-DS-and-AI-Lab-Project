...
BEST_MODEL_PATH = OUTPUT_ROOT / "best_cnn_gru_mps.pth"
HISTORY_CSV_PATH = OUTPUT_ROOT / "cnn_gru_training_history.csv"
TEST_REPORT_JSON_PATH = OUTPUT_ROOT / "cnn_gru_test_report.json"
CONFUSION_MATRIX_CSV_PATH = OUTPUT_ROOT / "cnn_gru_confusion_matrix.csv"
...
print("Sample batch sequence shape:", sample_x.shape)  # expected [B, 16, 3, 224, 224]
...
class CNNGRUModel(nn.Module):
    def __init__(self, hidden_size: int = 128, num_classes: int = 3):
        super().__init__()
    ...
    def forward(self, x):
        ...
        _, h_n = self.gru(frame_features)     # (1, B, hidden)
...
model = CNNGRUModel(num_classes=NUM_CLASSES).to(DEVICE)

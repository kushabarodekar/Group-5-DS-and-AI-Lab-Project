...
### Video Fatigue down-weighting (M6)

Video-Based Fatigue Detection is kept **enabled** but **down-weighted** in fusion:

- Locked final test accuracy ≈ **33.6%** (three-class; random baseline ≈ 33.3%)
- `RISK_FUSION_WEIGHTS["video_fatigue"]` = **0.10** (was 0.20)
- `VIDEO_FATIGUE_TRUST_FACTOR` = **0.50** scales its `R_i` before summation

It remains visible on the dashboard as a complementary experimental signal without
dominating the overall driver wellness score.

## Deployment

### Primary — Lightning AI / local / Hugging Face
...
See [start.txt](start.txt) for Lightning GPU setup.

### Fallback — Kaggle GPU (Milestone 6 live demo)

When Lightning free-tier GPU/runtime quotas may interrupt evaluation, use the
**Kaggle GPU fallback** (~9 h notebook session, public Gradio URL via `share=True`):
...
Full setup: **[docs/KAGGLE_FALLBACK.md](docs/KAGGLE_FALLBACK.md)**  
Notebook entry: **[kaggle/Risk_Fusion_M6_Fallback.ipynb](kaggle/Risk_Fusion_M6_Fallback.ipynb)**

Copy the log line **`Running on public URL:`** and share that link with evaluators.
Keep the Kaggle notebook cell running for the duration of the demo.
